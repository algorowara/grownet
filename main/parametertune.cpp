...
float utility(GrowingNetwork3D* net);

int main(){
	
	long int sample_size = 25;
	long int n = 100, m = 3;
	float base_gamma_step = 0.01, base_tol_step = 0.01;
	long int base_itr_step = 1;
	float gamma = 2.8, gamma_step = base_gamma_step, tolerance = 0.6, tol_step = base_tol_step;
	...
	float prev_util = utility(net);
	float best_gamma, best_tolerance;
	...
	while(gen < genmax){
		...
		#pragma omp parallel shared(best_gamma, best_tolerance, best_iterations, prev_util) private(net)
		{
			...
			for(long int i = 0; i < 8; i++){
				...
				float test_gamma = abs(gamma + ((i & 1<<0) ? -1:1) * gamma_step);
				float test_tolerance = abs(tolerance + ((i & 1<<1) ? -1:1) * tol_step);
