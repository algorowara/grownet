...
A key feature of the OHO model is that the nodes can only be placed on the circumference of the circle, when we generalize this to higher dimensions we are still embedding the network on an $d$-dimensional surface in an $d+1$ dimensional space, so nodes never occupy a volume in the embedding space.
In this generalization we explore how the network properties are affected if we instead embed a growing network in an $d$-dimensional without restricting them to a spherical shell.  
In order to ensure unbiased behavior over all nodes, we instead confine them to an $d$-dimensional ball. 
