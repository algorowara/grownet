...
In particular, we consider a class of models in which nodes are placed one by one in random locations in space and, after placement, form connections with nearby nodes. 
...
%the above claim is a bit too ambitious; it's not strictly true... we get a similar degree distribution even when the nodes are not particularly uniform (i.e. even if the method is unsound
...
%here it's not so much that Gauss' law continues to hold in general, it's that when we apply gauss's law in higher dimensions we find that the force exerted on nodes by the cloud scales linearly with r because the surface term from gauss' law cancels the all but the linear part of the volume term from the enclose charge.
...
%it's not clear what leveling off means in this context
The arrangement of nodes is best seen in the case of the 2-ball, the most photogenic of $d$-balls.  
%I think we should include a picture... maybe color coded by degree? Seems illustrative.
...
%end section we want to move, although the above sentence doesn't really work, not strictly true
...
%I'm not sure we want to use the word disequilibrium, it generally has connotations related to fields we're not working in.
