...
\caption{\label{pathlength}Semilogarithmic plot of characteristic graph path length ($\ell$) shows the desired scaling, $\ell \sim \log N$. Note that in general the average shortest path is shorter in the higher dimensions. The blue points represent the Sphere Network Model ($\alpha = d-1$), while the red points represent the Plum Pudding Network Model. The dimensions are represented as follows, 1D - circle; 2D - star; 3D - triangle; 4D - square.}
\end{figure}
...
\caption{\label{degdist}Semilogarithmic graph of the degree distribution for 8 models with $N = 10000$ and $m = 4$ compared to the theory in Eq. \eqref{degeq}. The slope of -0.22 agrees with the theoretical slope of $\log \frac{m}{m+1} = \log \frac{4}{5} \approx -0.22$. The blue points represent the Sphere Network Model while the red points represent the Plum Pudding Network Model. The dimensions are represented as follows, 1D - circle; 2D - star; 3D - triangle; 4D - square. Note that an arbitrary linear offset was used to separate data for visualization and thus values on the y-axis are only relative.}
\end{figure}
...
\label{sub:forcelaw}
FIXME: Write this and combine with the appendix

\section{CONCLUSION}
FIXME: Write conclusion.

\section{APPENDIX: FORCE LAWS} 
In calculating the gradients for the gradient-descent algorithm, we use a scaling taken from the Coulomb force in an arbitrary number of dimensions: $F \propto \frac{1}{r^{d-1}}$ in a $d$-dimensional space where $r$ is the distance between nodes.  
...
We chose these force laws for convenience (fairly simple straightforward force laws help easy computation) and because the natural analogy to electrostatics adds physical significance to our model. 
However, we found that altering the force law to similar repulsive forces (i.e. changing the dependence on $r$ by altering the exponent) did not produce significant changes in our results.
A stronger dependence on $r$, such as $F \propto \frac{1}{r^{d+2}}$ simply adds computational difficulty to the problem, but if the gradient descent method is implemented correctly, these distinct force laws lead to the same network properties as the model described in this paper.
Thus we find that our results apply not only to nodes behaving as electrons, but to any system where nodes interact through a repulsive force.
